## Unreleased
- Annotations: **Outcrop Traces** — take the mean attitude of an annotation selection (one annotation, several, or a whole group), repeat it at a constant bed thickness, and mark where that bedding sequence would crop out on the terrain. Built for stratigraphic correlation across sites: if two outcrops belong to the same sequence, the traces run continuously between them. Controls are in *Annotations → Outcrop Traces* — bed thickness and projection radius, both in metres, each with a *Fit* button that seeds it from the selection; trace width and smoothing are on the config page. See [docs/OutcropTraces.md](docs/OutcropTraces.md)
