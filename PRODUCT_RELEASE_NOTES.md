## Unreleased
- Annotations: **Outcrop Traces** — take the mean attitude of an annotation selection (one annotation, several, or a whole group), repeat it at a constant bed thickness, and mark where that bedding sequence would crop out on the terrain. Built for stratigraphic correlation across sites: if two outcrops belong to the same sequence, the traces run continuously between them. All controls are in *Annotations → Outcrop Traces*: bed thickness, projection radius and phase offset in metres — the first two with a *Fit* button that seeds them from the selection — with trace width, smoothing and colour under an *Appearance* divider below. **Phase offset** slides the whole sequence along the plane normal, which is how a modelled bed is lined up with a marker bed you can actually see. Numeric controls take millimetre values throughout, for thin beds and hairline traces. See [docs/OutcropTraces.md](docs/OutcropTraces.md)
- Annotations: the panel **says what attitude it is drawing** and refuses to draw when it should not. Planes are combined with the orientation tensor, the standard treatment for pole data, and its eigenvalue spectrum separates three cases: one dominant attitude (drawn, with `S₁` reported), scattered poles (*"no dominant attitude"*), and a girdle — meaning the **selection is folded**, where no single attitude represents it and the message names the fold axis instead. Averaging a fold's two limbs otherwise yields a confident plane perpendicular to both, which no single confidence number can flag
- Annotations: *Dip&Strike* gains a **Selection average** row — dip, dip direction and clustering for the whole multi-selection, not just the one annotation in focus. It shares the outcrop-trace computation, so the two panels cannot disagree, and it is available with outcrop traces switched off
- Annotations: outcrop traces do **not** appear in headless batch rendering (`PRo3D.Snapshots`) yet. The scene graph is shared, so interactive screenshots do include them; the settings are simply not saved with the scene
