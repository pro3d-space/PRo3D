...
- **Never create an adaptive node inside `AVal.custom` (or any token-driven evaluation) and read it with the token** — not `AVal.map`, not `adaptive {}`, not a helper returning an `aval`. Edges are weak and a custom owns nothing it creates: after a GC the dependency is gone and the value silently goes stale. Build it outside the closure, or use `bind`/CEs. See [ai/CONVENTIONS.md](ai/CONVENTIONS.md) rule 4.
