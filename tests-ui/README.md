...
| `double-click-finish` | double-click ends an annotation (#824, docs/DoubleClickFinish.md): real double-clicks on Dimorphos finish a polyline, polygon and dip & strike without a duplicate vertex, a too-early one lets drawing continue, projected segments stay one per pair, Point is unaffected, the tool strip swallows it, the per-computer Preferences switch turns it off (the spec backs up and restores `userPreferences.json`), Direct Tool Mode works without Ctrl and the classic scheme needs it, and a double-click applies a cut. Launches with `--remoteApi` and reads the saved `.ann` |
| `busy-indicator` | the busy indicator (docs/BusyIndicator.md) reports a stall *while* it is happening. The stall is a switch to ArcBall, which picks the centre ray against every surface synchronously — ~4 s on the Dimorphos data, cold. Through it, `/busy` keeps answering within 27 ms (so it is not behind `app.lock`) and the client-side overlay counts up `camera 0.1s … 3.9s`, then clears. An OPC import covers a second label. Also checks the idle contract, that the pill is click-through, and that `-nobusy` leaves nothing behind. Launches with `-busyms 1` |
