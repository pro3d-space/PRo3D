...
export function drawingScene(dir: string, name: string, edit: (d: any) => void = () => {}): string {
    return derivedScene(name, (d) => drawingSetup(d, edit), dir);
}

function drawingSetup(d: any, edit: (d: any) => void) {
    ...
    d.cameraView.view[1] = JSON.stringify(loc.map((c: number) => (c / len) * CAMERA_DISTANCE));
