...
**Scene body (#758, `docs/SceneBody.md`)** — `ReferenceSystem.planet` and the GIS observation (`observer` + `referenceFrame`) are **one setting**. Planet-based code (up/north, lat/lon, MapView's +Z pole and radius) reads world coordinates as the planet's body-fixed coordinates; that is only true when the GIS observes the body in its own fixed frame, which makes the scene body's surfaces' SPICE placement the identity.
- Change either only through `PRo3D.Viewer/SceneBodySync.fs` (`setPlanet`, `followObservation`, `reconcileOnLoad`) — never write `referenceSystem.planet` or `defaultObservationInfo.observer/referenceFrame` directly, or the two drift apart.
