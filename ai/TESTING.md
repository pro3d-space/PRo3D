...
- **`litFraction` does not reject the loading splash.** It measures the centre of
  the frame, which is exactly where the bright AARDVARK banner sits, and the tool
  strip keeps a corner pixel above `streamLive`'s threshold — so an unloaded view
  scores as "lit and stable" and the spec reads a half-loaded model. If a spec
  waits for the surface, gate on the splash colour too
  (`annotation-profile-export.spec.ts:splashFraction`).
...
- **The shared test scene carries the HERA AFC *instrument* camera — override it
  ...
  **hfov 5.53°** instead of PRo3D's 60°. The frustum is derived from *focal*, so
  overriding `frustum` alone does nothing — set `focal = 10.25` (and adjust the
