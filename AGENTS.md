...
| `adapt.cmd` / `adapt.sh` | Generate the missing/stale Adaptify `*.g.fs` (not checked in; `--all`, `--check`) — needed before an IDE or plain `dotnet build` |
...
- Generated files: `*.g.fs` (e.g. `Surface-Model.g.fs` next to `Surface-Model.fs`). They are **not checked in** (`.gitignore`) — every clone/worktree generates them locally.
- Generated by [`utilities/Adapt.fsx`](utilities/Adapt.fsx), which regenerates only projects whose `*.g.fs` are missing or stale (source hash in line 1). It runs from `adapt.cmd` / `adapt.sh`, from `build.cmd` / `build.sh` (FAKE target `Adapt`, before every compile/publish/test target) and from the `runTests`-style scripts. A plain `dotnet build` or an IDE build does **not** run it.
- **Fresh clone or new worktree:** run `adapt.cmd` / `adapt.sh` (or `build.cmd`) before building in an IDE; the first run builds Base/Core/GIS/SimulatedViews and takes minutes.
- **After changing a `[<ModelType>]`** (or pulling/switching to a change that did): run `adapt.cmd` / `adapt.sh`. A new model file `X.fs` also needs `<Compile Include="X.g.fs" />` right after it in the `.fsproj`.
- **DO NOT edit `*.g.fs` files** — change the source `.fs` model and regenerate. **Never `git add` them.**
- Details, limits and troubleshooting: [docs/ModelTypes.md](docs/ModelTypes.md).
...
├── PRo3D.MapProjection/   # Equirectangular / polar map panel for small bodies (library)
├── PRo3D.MapProjection.Standalone/  # Runs that panel as its own app (PRo3D.MapProjection.Standalone.exe)
...
3. **Never edit or commit `*.g.fs`** — they are local Adaptify output (`adapt.cmd` / `adapt.sh`).
...
| `*.g.fs` not found / type errors referencing `Adaptive*` types | Missing/stale `*.g.fs` (not checked in) | `adapt.cmd` / `adapt.sh`; if the model file itself is unchanged, `adapt.cmd --all` |
| `adapt` fails: "Adaptify did not produce up-to-date output" | Adaptify could not type-check the models (it still exits 0) | Rerun that project with `--verbose` as the message says — see [docs/ModelTypes.md](docs/ModelTypes.md) |
...
- When changing a model, remember the `*.g.fs` regeneration step (`adapt.cmd` / `adapt.sh`); in a fresh worktree run it before the first build.
