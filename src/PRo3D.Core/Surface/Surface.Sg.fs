...
module Sg =
    ...
    let createSceneGraphCanonical (signature : IFramebufferSignature) (scene : OpcScene) 
                                  (patchHierarchies : array<PatchHierarchy>) =
    ...
    let createSgSurface (scene : OpcScene) (s : Surface) (bb : Box3d) (kd : HashMap<Box3d,KdTrees.Level0KdTree>) (patchHierarchies : array<PatchHierarchy>) = 
        ...
        let sgSurface = {
                ...
                isObj       = false
                opcScene    = Some scene
                patchHierarchies = Some patchHierarchies
    ...
    let createSgSurfaces runtime signature (surfaces:IndexList<Surface>) =
        ...
        let sghs =
            ...
            |> List.map(fun (surf, (d, (sg, hierarchies, kdtree))) -> 
                ...
                { surf = surf; bb = bb; sg = sg; kdtree = kdtree ; scene = d; patchHierarchies = List.toArray hierarchies })
        
        let sgSurfaces =
          sghs 
          |> List.map (fun d -> createSgSurface d.scene d.surf d.bb d.kdtree d.patchHierarchies)
