...
module Files =
    ...
    let isOpcFolder' (quiet: bool) (path: string) =
        let isOpcFolder =
            let imagesProbingPaths =
                OpcPaths.Images_DirNames
                |> List.map (fun imageSuffix -> Path.combine [ path; imageSuffix ])

            let patchesProbingPaths =
                OpcPaths.Patches_DirNames
                |> List.map (fun patchSuffix -> Path.combine [ path; patchSuffix ])
    ...
    let isSurfaceFolder (path: string) =
        ...
        let containsOpcDir =
            ...
            | _ ->
                subdirs
                |> Seq.map (fun (x: string) -> isOpcFolder x)
                |> Seq.reduce (fun x y -> x || y)

        containsOpcDir

    let isRimfaxFolder (path: string) =
        let requiredExtensions = Set.ofList [ ".obj"; ".png"; ".mtl" ]

        Directory.GetFiles(path)
        |> Seq.map Path.GetExtension
        |> Set.ofSeq
        |> Set.isSubset requiredExtensions

    /// <summary>
    /// Returns the first file with extension .obj in a folder
    /// </summary>
    let getObjFilePath (path: string) =
        Directory.GetFiles(path)
        |> Seq.tryFind (fun file -> Path.GetExtension(file).ToLower() = ".obj")
    ...
    let discover (p: string -> bool) path : list<string> =
        if Directory.Exists path then
            Directory.EnumerateDirectories path |> Seq.filter p |> Seq.toList
        else
            List.empty

    /// returns all valid surface folders in "path"
    let discoverSurfaces path = discover isSurfaceFolder path

    let discoverOpcs path = discover isOpcFolder path
    ...
    let whyEscaping (path: string) = path
    //path.Replace(@"\", @"\\\\")


    let private retrieve (f: string -> string) (path: string) : list<string> =
        if path = String.Empty then
            failwith "encountered empty opc path"
    ...
    let rec superDiscovery (input: string) : string * list<string> =
        ...
        | Directory path ->
            let opcs =
                ...
                |> Seq.toList
                |> List.map (fun x -> superDiscovery x |> snd)
                |> List.concat

            input, opcs
        | OpcFolder p -> input, [ p ]

    /// Recursively finds all directories under the given input path that satisfy the provided predicate.
    /// The predicate is applied to each directory, and matching directories are collected in the result.
    let rec discoverMatchingFolders (folderPredicate: string -> bool) (input: string) : list<string> =
        match input |> folderPredicate with
        | true ->            
            [ input ]
        | _ ->
            input
            ...
            |> Seq.toList
            |> List.map (fun x ->                
                discoverMatchingFolders folderPredicate x)
            |> List.concat
    let discoverSurfaceFolders (input: string) : list<string> =
        discoverMatchingFolders isSurfaceFolder input

    let discoverRimfaxFolders (input: string) : list<string> =
        discoverMatchingFolders isRimfaxFolder input
   
    /// <summary>
    /// returns all valid OPCs in "path"
    /// </summary>
    let getOPCPaths path = path |> retrieve (id)

    /// <summary>
    /// returns all valid OPCs in "path"
    /// </summary>
    let getOPCNames path = path |> retrieve (Path.GetFileName)
    ...
    let relativePath (path: string) (remaining: int) =
        ...
        if parts.Length > remaining then
            Array.skip (parts.Length - remaining) parts |> Path.combine |> Some
        else
            None

    let sceneRelativePath (path: string) = relativePath path 5

    let surfaceRelativePath (path: string) = relativePath path 4

    let expandLazyKdTreePaths
        (scenePath: option<string>)
        (surfaces: HashMap<Guid, Surface>)
        ...
        let expand surf tree =
            ...
            | KdTrees.Level0KdTree.LazyKdTree lk when surf.relativePaths && scenePath.IsSome -> // scene is portable
                ...
                | _ ->
                    Log.warn
                        "[expandLazyKdTreePaths] could not create relative paths for %A"
                        (lk.kdtreePath, lk.objectSetPath)
        ...
        | Some sp ->
            ...
            | Some s ->
                ...
                | Leaf.Surfaces sf ->
                    ...
                    let s' =
                        { sf with
                            opcPaths = expandNamesToPaths targetDir sf.opcNames
                            relativePaths = true }
