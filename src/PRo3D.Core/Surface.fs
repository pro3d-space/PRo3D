                    ...
                    if kd.IsEmpty then Log.error "no kdtree loaded for %s" surf.name; None
                        ...
                        let closestHit =
                            ...
                            |> List.choose(fun key -> 
                                ...
                                match hit with
                                | Some (t, object) -> 
                                    let hitPointTransformed = ray.Ray.GetPointOnRay(t)
                                    let hitPointOriginal = backward.Inverse.TransformPos(hitPointTransformed)
                                    let tOfOriginalRay = r.Ray.GetTOfProjectedPoint(hitPointOriginal)
                                    cache <- c
