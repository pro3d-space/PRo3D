...
module ProfileAttributeExtraction =
    ...
    ///
    /// The texture fallback is enabled: unlike the 3D cursor this runs in bulk and
    /// off the render loop, so an image decode per layer per patch is affordable
    /// and it is worth covering layers the per-vertex data misses.
    let sampleAt
        (up             : V3d)
        ...
        | Some hitInfo, cache ->
            ...
            let scalarLayers =
                ...
                | None      -> HashMap.empty
            extractAttributesFromHit (TextureFallback.Enabled scalarLayers) hitInfo ray, cache
