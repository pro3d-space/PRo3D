//13697e27-5ff3-babb-b285-45789255a539
//67388f21-daa1-0433-9750-f24f48fb0af1
...
[<System.Diagnostics.CodeAnalysis.SuppressMessage("NameConventions", "*")>]
type AdaptiveSceneHandle(value : SceneHandle) =
    let _path_ = FSharp.Data.Adaptive.cval(value.path)
    let _name_ = FSharp.Data.Adaptive.cval(value.name)
    let _writeDate_ = FSharp.Data.Adaptive.cval(value.writeDate)
    let mutable __value = value
    let __adaptive = FSharp.Data.Adaptive.AVal.custom((fun (token : FSharp.Data.Adaptive.AdaptiveToken) -> __value))
    static member Create(value : SceneHandle) = AdaptiveSceneHandle(value)
    static member Unpersist = Adaptify.Unpersist.create (fun (value : SceneHandle) -> AdaptiveSceneHandle(value)) (fun (adaptive : AdaptiveSceneHandle) (value : SceneHandle) -> adaptive.Update(value))
    member __.Update(value : SceneHandle) =
        if Microsoft.FSharp.Core.Operators.not((FSharp.Data.Adaptive.ShallowEqualityComparer<SceneHandle>.ShallowEquals(value, __value))) then
            __value <- value
            __adaptive.MarkOutdated()
            _path_.Value <- value.path
            _name_.Value <- value.name
            _writeDate_.Value <- value.writeDate
    member __.Current = __adaptive
    ...
    member __.writeDate = _writeDate_ :> FSharp.Data.Adaptive.aval<System.DateTime>
[<AutoOpen; System.Diagnostics.CodeAnalysis.SuppressMessage("NameConventions", "*")>]
module SceneHandleLenses = 
    type SceneHandle with
        static member path_ = ((fun (self : SceneHandle) -> self.path), (fun (value : Microsoft.FSharp.Core.string) (self : SceneHandle) -> { self with path = value }))
        static member name_ = ((fun (self : SceneHandle) -> self.name), (fun (value : Microsoft.FSharp.Core.string) (self : SceneHandle) -> { self with name = value }))
        static member writeDate_ = ((fun (self : SceneHandle) -> self.writeDate), (fun (value : System.DateTime) (self : SceneHandle) -> { self with writeDate = value }))
[<System.Diagnostics.CodeAnalysis.SuppressMessage("NameConventions", "*")>]
...
type AdaptiveModel(value : Model) =
    ...
    let _animations_ = Aardvark.UI.Animation.AdaptiveAnimationModel(value.animations)
    ...
    let _frustumModel_ = PRo3D.AdaptiveFrustumModel(value.frustumModel)
...
module ModelLenses = 
    type Model with
        ...
        static member animations_ = ((fun (self : Model) -> self.animations), (fun (value : Aardvark.UI.Animation.AnimationModel) (self : Model) -> { self with animations = value }))
        ...
        static member frustumModel_ = ((fun (self : Model) -> self.frustumModel), (fun (value : PRo3D.FrustumModel) (self : Model) -> { self with frustumModel = value }))
        ...
        static member animator_ = ((fun (self : Model) -> self.animator), (fun (value : Aardvark.UI.Anewmation.Animator<Model>) (self : Model) -> { self with animator = value }))
