...
let startBatchRendering (startupArgs : PRo3D.SimulatedViews.CLStartupArgs) = 
...
let main argv = 
    ...
    //TODO - combine startup arguments of batch rendering tool and normal viewer
    let doBatchrendering = (argv |> CommandLineUtils.hasFlag "batch")
    if doBatchrendering then
        startBatchRendering (SimulatedViews.CommandLine.parseArguments argv)
        0
    else
