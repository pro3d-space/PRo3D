...
//
// Verbatim copy of src/Aardvark.Geometry/PolyRegion2d.fs from
// aardvark-platform/aardvark.base at commit 0420fd19bdf3ca27e9bd6c77036afbabf652e50f
// ("Add attributed PolyRegion boolean operations", fixes aardvark.base#100).
//
// The only change is the namespace on the next line: PRo3D.Base already references the real
// Aardvark.Geometry, so keeping the upstream namespace would make every PolyRegion use site
// ambiguous.
//
// We carry this copy because 0420fd19 is on master but in no tag (latest 5.3.27) while PRo3D
// resolves Aardvark.Geometry 5.3.26, and boolean operations cannot wait for a release.
//
// TODO: delete this file and switch back to Aardvark.Geometry once a release carries 0420fd19.
// See https://github.com/aardvark-platform/aardvark.base/issues/100
//
// Keeping it byte-identical apart from the namespace is what makes that deletion a clean revert
// - so do not reformat, tidy or "improve" anything below.
