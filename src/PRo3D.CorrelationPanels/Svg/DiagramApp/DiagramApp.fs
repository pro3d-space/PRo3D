...
module DiagramApp =         
    ...
    let layoutDiagramItems (model : DiagramAppModel) =
        ...
        let cleanItems items =
            items
            |> HashMap.map (fun id s -> DiagramItem.resetPosition s (V2d 0.0))
            |> HashMap.update 
    ...
    let tryFindRectangle (model: DiagramAppModel) (id: RectangleId) =
        ...
        |> Seq.fold(fun acc x -> x |> HashMap.union acc) HashMap.empty
        |> HashMap.tryFind id
    ...
    let tryGetIdPair (model: DiagramAppModel) (id: RectangleId) =
        ...
        |> Seq.map(fun (k,v) -> v.rectangles  |> HashMap.map(fun _ r -> (k, v.id)))
        |> Seq.fold(fun acc x -> x |> HashMap.union acc) HashMap.empty
        |> HashMap.tryFind id
                        ...
                        primaryStack = { 
                            d.primaryStack with 
                                rectangles = 
                                    ...
                                    |> HashMap.map(fun _ r -> Rectangle.update r a)
                        }
                        secondaryStack = 
                            match d.secondaryStack with
                            | None -> d.secondaryStack
                            | Some s -> Some { s with rectangles = s.rectangles |> HashMap.map(fun _ r -> Rectangle.update r a) }
                })  
        { model with items = items }
    ...
    let updateDiagramItem diagramItemId model a =
        {
            model with 
                items = 
                    model.items
                    |> HashMap.alter diagramItemId (fun item ->
                        match item with
    ...
    let deselectAllRectangleBorders model =
        ...
        { model with selectedBorders = HashMap.empty; items = HashMap.union model.items updatedItems }      
    ...
    let update (msg: DiagramAppAction) (model: DiagramAppModel) =
        ...
        let model =
            ...
            | UpdateItem diagramItem ->
                ...
                let updatedItems =
                    model.items 
                    |> HashMap.alter diagramItem.id (function
                        | Some item ->                             
            ...
            | DiagramItemMessage (diagramItemId, a) ->
                ...
                | RectangleStackMessage(_, SelectBorder _) ->
                    ...
                    //Log.line "[DiagramApp] selected borders %A" (blurg |> HashSet.toList)
            ...
            | SetYScaling scaleValue ->
                ...
                let stacks =
                    model.items
                    |> HashMap.map (fun _ r -> DiagramItemApp.update r (DiagramItemAction.SetYScaling yPixelPerUnitSvg))

                { model with yScaleValue = scaleValue; yToSvg = yPixelPerUnitSvg; items = stacks } |> layoutDiagramItems
            | UpdateXSizes f ->
                let stacks =
                    model.items
                    |> HashMap.map (fun _ r -> DiagramItemApp.update r (DiagramItemAction.UpdateXSizes f))
            ...
            | CorrelationsMessage a -> 
                ...
                | CorrelationsAction.FlattenHorizon correlationId ->
                    let referenceCorrelation = model.correlations.correlations |> HashMap.find correlationId
                    let itemOffsets = 
                        referenceCorrelation.contacts 
                        |> HashMap.choose (fun rectanlgeBorderId borderContactId ->
                            let rectangelBorder = model.bordersTable |> HashMap.find rectanlgeBorderId
                            let upperRectangleId = rectangelBorder.upperRectangle 
                            match tryGetIdPair model upperRectangleId with 
                            | Some (itemId, stackId) -> 
                                let diagramItem = model.items |> HashMap.find itemId
                    ...
                    let items = itemOffsets |> HashMap.values
                    if items.IsEmpty() then
                        ...
                        let updatedItems = 
                            ...
                            oldMap 
                            |> HashMap.alter diagramItemId (fun optionItem ->
                ...
                | CorrelationsAction.DefaultHorizon -> 
                    let updatedItems = model.items |> HashMap.map (fun _ item -> {item with flattenHorizon = None })
                    { model with items = updatedItems }
                | CorrelationsAction.Select _ ->
                    ...
                    let selectedCorrelation = 
                        ...
                        |> Option.bind(fun x ->
                            model.correlations.correlations |> HashMap.tryFind x
                        )
                    ...
                    | Some c ->
                        ...
                        let updatedItems = 
                            ...
                            |> List.choose(fun rectangleBorderId -> 
                                ...
                                |> List.choose(fun (itemId, item) ->
                                    (item.primaryStack.borders |> HashMap.tryFind rectangleBorderId)
                        ...
                        { model with selectedBorders = c.contacts; items = HashMap.union model.items updatedItems }
                    | None ->
    ...
    let createCorrelation (model : DiagramAppModel) =
        ...
        //{ model with correlations = corr; selectedBorders = HashMap.empty }
    ...
    let init : DiagramAppModel =
...

        {
            ...
            correlations       = { version = CorrelationsModel.current; correlations =  HashMap.empty; selectedCorrelation = None; alignedBy = None } //CorrelationsModel.init
            selectedRectangle  = None
            dataRange          = Range1d.Invalid
            selectedBorders    = HashMap.empty
            bordersTable       = HashMap.empty
            rectanglesTable    = HashMap.empty
