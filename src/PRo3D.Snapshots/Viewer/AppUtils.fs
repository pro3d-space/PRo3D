...
/// already integrated in Aardvark Media, should be replaced with call
module AppExtension =
    //type MutableApp

    type App<'model, 'mmodel, 'msg> 
        with member app.startAndGetState() =
                let l = obj()
    ...
    let start' (app : App<'model, 'mmodel, 'msg>) = app.startAndGetState ()
