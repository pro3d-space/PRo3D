...
module ViewPlan = 
    ...
    let updateViewPlans (vp:ViewPlan) (vps:HashSet<ViewPlan>) =
        vps
        |> HashSet.map (fun x -> if x.id = vp.id then vp else x)
    ...
    let calculateSurfaceContactPoint (roverWheel:V3d) (refSystem : ReferenceSystem.ReferenceSystem) 
                                     (cache : HashMap<string, ConcreteKdIntersectionTree>) (trafo:Trafo3d)
    ...
    let createViewPlanFromFile (data:HashMap<string,string>) (model:ViewPlanModel) (rover:Rover)
        ...
        let pos = V3d((data |> HashMap.find "X").ToFloat(), 
                      (data |> HashMap.find "Y").ToFloat(), 
                      (data |> HashMap.find "Z").ToFloat())
        
        let posQ = V4d((data |> HashMap.find "qX").ToFloat(), 
        ...
        { model with viewPlans = HashSet.add newViewPlan model.viewPlans; 
    ...
    let updateRovers (model:ViewPlanModel) (roverModel:RoverModel) (vp:ViewPlan) (fp:FootPrint)  : ('a*ViewPlanModel)=
        ...
        let m'= { model with selectedViewPlan = Some vp'; 
                             viewPlans = model.viewPlans |> HashSet.map(fun x -> if x.id = vp.id then vp' else x) }
        ...
        match action with
                    ...
                    | Some r -> 
                        match model.working.IsEmpty with
                            ...
                            | false -> // second point (lookAt)
                                ...
                                outerModel, { model with viewPlans = HashSet.add wp model.viewPlans; 
            ...
            | IsVisible id -> 
                outerModel, { model with viewPlans = 
                                         model.viewPlans 
                                            |> HashSet.map(fun x -> if x.id = id then 
                    ...
                    | Some vp -> let newVp = { vp with selectedInstrument = i }
                                 ...
                                 newOuterModel, { m' with selectedViewPlan = Some newVp; 
                                                      viewPlans = model.viewPlans 
                                                                    |> HashSet.map(fun x -> if x.id = vp.id then 
                                                                                            newVp 
                                                                                            else x) }
                    | None -> outerModel, model                                                     
            | SelectAxis a       -> 
                ...
                match model.selectedViewPlan with
                    | Some vp -> 
                        ...
                        | Some inst ->   
                            ...
                            let instruments' = 
                                vp.rover.instruments |> HashMap.update id (fun x -> 
                                    match x with
                                        | Some _ -> inst'
        ...
        let drawWheels (vp:MViewPlan) (cam:aval<CameraView>) =
            alist {
                ...
                for w in wheels do
                    ...
                    yield Sg.ofList [
                            ReferenceSystemApp.Sg.point wheelPos (AVal.constant C4b.White) cam ]   
                }
