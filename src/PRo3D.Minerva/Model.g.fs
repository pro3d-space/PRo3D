//d18ae8ca-5581-d5c2-3b82-103302d78870
//86898f7b-bc7a-21ef-2f2f-b17b448ce11f
...
type AdaptiveFeatureProperties(value : FeatureProperties) =
    let _pointSize_ = Aardvark.UI.AdaptiveNumericInput(value.pointSize)
...
module FeaturePropertiesLenses = 
    type FeatureProperties with
        static member pointSize_ = ((fun (self : FeatureProperties) -> self.pointSize), (fun (value : Aardvark.UI.NumericInput) (self : FeatureProperties) -> { self with pointSize = value }))
        static member textSize_ = ((fun (self : FeatureProperties) -> self.textSize), (fun (value : Aardvark.UI.NumericInput) (self : FeatureProperties) -> { self with textSize = value }))
[<System.Diagnostics.CodeAnalysis.SuppressMessage("NameConventions", "*")>]
type AdaptiveQueryModel(value : QueryModel) =
    let _version_ = FSharp.Data.Adaptive.cval(value.version)
    let _minSol_ = Aardvark.UI.AdaptiveNumericInput(value.minSol)
    let _maxSol_ = Aardvark.UI.AdaptiveNumericInput(value.maxSol)
    let _distance_ = Aardvark.UI.AdaptiveNumericInput(value.distance)
...
module QueryModelLenses = 
    type QueryModel with
        ...
        static member minSol_ = ((fun (self : QueryModel) -> self.minSol), (fun (value : Aardvark.UI.NumericInput) (self : QueryModel) -> { self with minSol = value }))
        static member maxSol_ = ((fun (self : QueryModel) -> self.maxSol), (fun (value : Aardvark.UI.NumericInput) (self : QueryModel) -> { self with maxSol = value }))
        static member distance_ = ((fun (self : QueryModel) -> self.distance), (fun (value : Aardvark.UI.NumericInput) (self : QueryModel) -> { self with distance = value }))
