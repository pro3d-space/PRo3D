//c0577544-57ed-5d4c-1d8e-c3a6e82b9044
//9f4d4086-1eb5-3dd6-d3a7-4fbb4066cd9e
...
type AdaptiveShadingApp(value : ShadingApp) =
    ...
    let _lightLocation_ = Aardvark.UI.AdaptiveV3dInput(value.lightLocation)
    let _lightDirection_ = Aardvark.UI.AdaptiveV3dInput(value.lightDirection)
    let _normalizeLightDirection_ = FSharp.Data.Adaptive.cval(value.normalizeLightDirection)
    let _lightDistance_ = Aardvark.UI.AdaptiveNumericInput(value.lightDistance)
    ...
    let _ambient_ = Aardvark.UI.AdaptiveNumericInput(value.ambient)
    let _ambientShadow_ = Aardvark.UI.AdaptiveNumericInput(value.ambientShadow)
    let _shadowFrustum_ = Aardvark.UI.AdaptiveNumericInput(value.shadowFrustum)
...
module ShadingAppLenses = 
    type ShadingApp with
        ...
        static member lightLocation_ = ((fun (self : ShadingApp) -> self.lightLocation), (fun (value : Aardvark.UI.V3dInput) (self : ShadingApp) -> { self with lightLocation = value }))
        static member lightDirection_ = ((fun (self : ShadingApp) -> self.lightDirection), (fun (value : Aardvark.UI.V3dInput) (self : ShadingApp) -> { self with lightDirection = value }))
        ...
        static member lightDistance_ = ((fun (self : ShadingApp) -> self.lightDistance), (fun (value : Aardvark.UI.NumericInput) (self : ShadingApp) -> { self with lightDistance = value }))
        ...
        static member ambient_ = ((fun (self : ShadingApp) -> self.ambient), (fun (value : Aardvark.UI.NumericInput) (self : ShadingApp) -> { self with ambient = value }))
        static member ambientShadow_ = ((fun (self : ShadingApp) -> self.ambientShadow), (fun (value : Aardvark.UI.NumericInput) (self : ShadingApp) -> { self with ambientShadow = value }))
        static member shadowFrustum_ = ((fun (self : ShadingApp) -> self.shadowFrustum), (fun (value : Aardvark.UI.NumericInput) (self : ShadingApp) -> { self with shadowFrustum = value }))
