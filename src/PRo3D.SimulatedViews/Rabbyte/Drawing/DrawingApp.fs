...
//    let allSegmentPoints (segments: alist<AdaptiveSegment>) : alist<V3d> = 
//        let lastPoint = 
//            segments 
//            |> AList.tryLast
//            |> AVal.map (fun x -> x
//                                  |> Option.map (fun x -> x.endPoint |> AList.ofAValSingle)
...
//        let allButLast = 
//            segments 
//            |> AList.map (fun x -> (AList.ofAValSingle x.startPoint) |> AList.append x.points) 
//            |> AList.collect id

//        AList.append allButLast lastPoint


//    let drawContourWithPointSize (points: alist<V3d>) (segments: alist<AdaptiveSegment>) (style: AdaptiveBrushStyle) (near: aval<float>) (far: aval<float>) (pointSize: aval<float>) (depthOffset : aval<float>)=  
...
//    let drawContour (points: alist<V3d>) (segments: alist<AdaptiveSegment>) (style: AdaptiveBrushStyle) (near: aval<float>) (far: aval<float>) =  
//        drawContourWithPointSize points segments style near far (AVal.constant 10.0) (AVal.constant 0.1)

//    let view (near: aval<float>) (far: aval<float>) (model: AdaptiveAnnotation)  = 
//        drawContourWithPointSize model.points 
//                                 model.segments
//                                 model.style near far (AVal.constant 10.0) (AVal.constant 0.1) |> Sg.noEvents

//    //let viewPointSize (near: aval<float>) (far: aval<float>) (pointSize: aval<float>) (depthOffset: aval<float>) (model: AdaptiveDrawingModel) = 
//    //    drawContourWithPointSize model.points
//    //                             model.segments 
//    //                             model.style near far pointSize depthOffset |> Sg.noEvents
