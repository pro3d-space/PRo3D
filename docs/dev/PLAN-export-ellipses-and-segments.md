...
- **Persistence (revised after review).** Older releases read `ellipseResults` with a reader that *requires* `center` / `major` / `minor` (the lon/lat ellipse) and fail the whole scene otherwise. So `ellipseResults` keeps exactly that schema and is only written when a lon/lat ellipse exists, as before; the metric shape is a separate annotation key, `ellipseShape`, which older releases skip. In memory both stay one `EllipticAnnotationResult` (`geographicalEllipse` optional). Scenes saved by this build open in older releases; that is a hard rule, not a trade-off.
