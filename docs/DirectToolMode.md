...
| finish an annotation / apply a cut by double-click (`DoubleClickFinish`) | `toolArmed` **&&** interaction ∈ {`DrawAnnotation`, `CutAnnotation`} **&&** config `doubleClickFinishes` — `ViewerApp.updateViewer`, see [DoubleClickFinish.md](DoubleClickFinish.md) |
