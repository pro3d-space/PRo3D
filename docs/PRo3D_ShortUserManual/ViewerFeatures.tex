...
The mission time settings, shown in Figure \ref{label:mission_phases} allow the user to choose a mission time frame and to navigate through the simulation using the slider. The mission phases include the Deimos Flyby, the Mars Flyby and the Didymos Orbital Insertion. 
...
Simulated images the images must contain a json file with the same name and the suffix .mbi, i.e. <image\_name>.mbi.json, which stores information about the instrument, the time and the target body. This file should contain at least the following fields:
...
Those are required to project the image onto a celestial body. 
