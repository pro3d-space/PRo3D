...
Figure \ref{fig:image_projection} shows the settings the user can adjust to control the image projection onto a body. The \textit{visualization} slider controls the opacity of the image projection and the registration properties \textit{roll}, \textit{pitch} and \textit{yaw} register the orientation of the projected image. The visibility can be set to \textit{RelativeCount}. The sun / lighting mode provides the options \textit{Off, SunDirect} and \textit{SunShadow}.
...
\begin{figure}[h!]
    \subfigure{
        \includegraphics[width=0.499\textwidth]{./pics/AnnotationEllipse1.PNG}
    }
    \hfill
    \subfigure{ 
        \includegraphics[width=0.499\textwidth]{./pics/AnnotationEllipse2.PNG}
	}
    \caption{AxisEllipse Annotation}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{pics/crossSectionTeaser.png}
	\caption{A cross section with a mapped curtain image.}
\end{figure}

\paragraph{1. Create a polyline annotation.}
Use sky projection and adequate subsampling if needed. Watch out not to generate insane segment counts. For example, for long cross-sections increase the sampling distance.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{pics/crossSectionAnno.png}
	\caption{The polyline annotation that defines the cross section.}
\end{figure}

\paragraph{2. Profile Export}
Export the profile of the annotation.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{pics/exportProfile.png}
	\caption{Exporting the profile of the annotation.}
\end{figure}

The exported profile will look like this:
...
Now use the draw profile tool to create an SVG/PNG which shows the profile:
...
\begin{itemize}
	\item Choose curtain height to match the full vertical span you need.
	\item \texttt{min-altitude} should be where the profile ends (lowest point of the profile).
	\item Curtain height is the height in meters.
	\item All of these need to be matched up later when setting the curtain details (step~6).
	\item The \texttt{vertical-px} parameter is used as the height of the image. The width is computed automatically given the profile length. For long profiles you might want to lower the resolution, as later in PRo3D the width must not exceed 16384 pixels.
\end{itemize}

Look at the profile or the annotation info in PRo3D to find good parameters (e.g.~the vertical span). Use the approximate curtain height and minimum altitude to specify the 2D viewport of the cross section.

\paragraph{3. Interpretation and tweaking}
Do your interpretation and tweaking of the cross section. Next, convert it to a PNG and remember the file name.

\paragraph{4. Creating the cross section}
Go to the annotation properties and move the camera far away from the cross section. Everything between the annotation and the point of view when creating the cross section will be clipped away.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{pics/createCrossSection.png}
	\caption{Creating the cross section from the annotation properties.}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{pics/clippedCrossSection.png}
	\caption{The clipped data along the cross section.}
\end{figure}

\paragraph{5. Setting curtain details}
Next, choose the curtain settings and set up the curtain.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{pics/curtainProperties.png}
	\caption{The curtain properties.}
\end{figure}

\paragraph{6. Inspection of cross section and curtain}

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{pics/crossSectionAndCurtain.png}
	\caption{Inspection of the final cross section and curtain.}
\end{figure}
