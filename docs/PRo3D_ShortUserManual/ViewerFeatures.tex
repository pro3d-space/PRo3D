...
\furtherreading{\onlinedoc{VertexAttributes.md}{VertexAttributes.md} --- attribute layers
that newer OPC exports additionally store per vertex, and what that changes for scalars and
measurements; \onlinedoc{Transformations.md}{Transformations.md} --- translating, rotating
and scaling a surface; \onlinedoc{SurfaceDistanceFilter.md}{SurfaceDistanceFilter.md} ---
hiding geometry beyond a given radius; \onlinedoc{KdTrees.md}{KdTrees.md} --- the kd-trees
behind picking, and what the \textit{Cloud} button rebuilds.}
...
Beyond these numbers, annotations can be written out point by point via
\textit{Annotations $\rightarrow$ Export}. Such an export turns a line or polyline into a
\textbf{profile}: one row per point, carrying the distance along the line and the height at
that point. If the surface ships attribute layers, the values of those layers underneath
each point can be exported alongside, which yields a \textbf{multi-attribute profile} ---
the whole cross section of the terrain in a single table, ready to plot.

\furtherreading{\onlinedoc{MultiAttributeProfile.md}{MultiAttributeProfile.md} --- a
walkthrough of a multi-attribute profile on the HERA Dimorphos dataset, from drawing the
line to the exported table; \onlinedoc{AnnotationExport.md}{AnnotationExport.md} --- the
reference description of every control in the export window.}
...
\furtherreading{\onlinedoc{ColorByCategory.md}{ColorByCategory.md} --- coloring annotations
by one of their attributes instead of by their own color;
\onlinedoc{AnnotationBooleanOps.md}{AnnotationBooleanOps.md} --- union and cutting of closed
annotations; \onlinedoc{CrossSections.md}{CrossSections.md} --- cutting surfaces along a
polyline annotation and mapping images onto the cutting plane;
\onlinedoc{OutcropTraces.md}{OutcropTraces.md} --- deriving from a dip-and-strike annotation
where a bedding sequence crops out on the terrain;
\onlinedoc{SbmtImport.md}{SbmtImport.md} --- importing annotation catalogs exported from the
...
\begin{figure}[h]
	...
	\label{fig:SequencedBookmarks_tabSnapshots}
\end{figure}
...
This feature allows to interpret celestial bodies and surfaces
from within PRo3D and serves as a basis for GIS functionality in PRo3D.
Within the GIS UI tab the user can assign coordinate frames and
celestial body information to surfaces and to GIS entities. Fly-by scenarios can be modeled by setting the observation time and choosing an observer body.

\furtherreading{\onlinedoc{GisView.md}{GisView.md} --- the continuously updated version of
this section, including sun illumination and cast shadows;
\onlinedoc{SceneBody.md}{SceneBody.md} --- the celestial body a scene is expressed in, and
how it relates to the reference frames chosen here;
\onlinedoc{spice.md}{spice.md} --- how SPICE is integrated into PRo3D and which kernels it
ships with.}
...
Pro3d provides the option to load custom spice kernels. In this documentation, we use the
\href{https://s2e2.cosmos.esa.int/bitbucket/projects/spice_kernels/repos/hera/browse}{HERA spice kernels}, which are also used in the
...
\hypertarget{the-command-line}{%
	\paragraph{Option 1: The command line}\label{the-command-line}}
...
\hypertarget{the-ui}{%
	\paragraph{Option 2: The UI}\label{the-ui}}

\begin{enumerate}
	\def\labelenumi{\arabic{enumi}.}
	\tightlist
	\item
	PRo3D with GIS view enabled looks like this initially:
\end{enumerate}
...
After loading the kernel, its path is automatically inserted in the settings pane within the GIS view.
...
was not successful. PRo3D's text output will provide more detailed information about the error, e.g. if the kernel path is incorrect.
...
Observation Settings''. This is used to define the observed target, a camera location, a point in time, and a reference frame.
...
\begin{itemize}
	...
	\item
	Camera Source body: The location of the camera looking at the observed body.
	\item
	...
	\item
	Reset: Re-applies the camera settings for the current observation, i.e. sets the camera to the location of the observer and points it towards the observed body.
\end{itemize}
...
with a reference frame and can be associated with an entity. The
reference frame is required to transform the surface to the global reference system used by PRo3D. The assignment is done in the ``Surfaces'' tab.
...
dropdown menu. If the required entity or reference frame is not in the
...
Entities can be celestial bodies or an spacecraft. To work with PRo3D's GIS
functionality, the loaded spice kernel needs to define the selected Spice Name of the entity.

The entity tab lists all entities. Some default entities are already set in PRo3D.

Each entity can set a reference frame in the column ``Reference
Frame''. Each entity can be represented by a proxy geometry, which is activated in the ''Proxy'' column, circled in blue in Figure \ref{fig:entitiesWithCircles}. 

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{pics/entities.png}
	\caption{Entities in the GIS view.}
    \label{fig:entitiesWithCircles}
\end{figure}

An entity can be edited by clicking on the \textit{edit} icon in the
``Actions'' column. The icon used to edit the HERA entity is circled in
red in Figure \ref{fig:entitiesWithCircles}.

Clicking the \textit{edit} icon opens a section for the entity, shown in Figure \ref{fig_editEntity}:

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{pics/editEntity.png}
	\caption{Editing Entities.}
    \label{fig_editEntity}
\end{figure}

The user can set the reference frame and choose whether to draw the proxy geometry. They can as well set the following settings:
...
\begin{itemize}
	...
	\item
	\textbf{Radius} The radius of the proxy sphere that is drawn in the location
	of the entity in meters.
    \item
	\textbf{Show Trajectory} Toggle the display of a celestial body's movement path, showing the trajectory it follows over time.
    \item
	\textbf{Trajectory Length} The number of days that the trajectory represents.
	of the entity in meters.
\end{itemize}
...
The red \textit{cross} icon in the ``Actions'' column deletes the corresponding
entity.

A new entity is created with the green \textit{plus} icon at the
...
created. To change a spice name you need to delete the old entity and
create a new one with the new spice name and the settings of the old
entity. Spice names are unique; It is not possible to create two entities with
the same spice name.
...
Reference frames can be deleted (\textit{red cross} icon) and created (\textit{green plus} icon in the bottom right corner) in this view, shown in Figure \ref{fig:referenceFrames}.
...
The spice name can only be set when a reference frame is created. To change a spice name, the old reference frame must be deleted and a new one needs to be generated. Spice names are unique; it is not possible to create two reference frames with the same spice name.

\hypertarget{observing-mars}{%
	\subsubsection{Observing mars}\label{observing-mars}}

In the following demonstration we observe Mars from Phobos.

1. Set the observation settings (including a time which is in available in the kernel) as shown in Figure \ref{fig:observationSettings}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{pics/observe.png}
	\caption{Observing Settings.}
    \label{fig:observationSettings}
\end{figure}

2. Ensure the proxy visualization for Mars is enabled, as indicated in Figure \ref{fig:entitiesWithCircles} with the blue circle.

3. Adjust the near \emph{and} far planes to ensure that Mars and Phobos are visible in the rendering as shown in Figure \ref{fig_nearAndFarPlanes}.
...
\begin{figure}[h!]
    ...
    \label{fig_nearAndFarPlanes}
\end{figure}

4. Optionally choose a texture for the entity, shown in Figure \ref{fig_entityTexture}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=1\textwidth]{./pics/textured.png}
	\caption{Textures can be specified and mapped onto entities.}
    \label{fig_entityTexture}
\end{figure}

5. To replace the proxy geometry with a real Mars model load the mola dataset. Specify a reference frame and a celestial body in the Surfaces pane within the Gis View (if it does not appear change the observation settings, e.g.~by setting the time).

6. Since we have a full surface for Mars now, we can disable the proxy geometry, shown in Figure \ref{fig:disableProxy}.
...
\begin{figure}[h!]
    ...
    \subfigure{
        \centering
        \includegraphics[width=\textwidth]{./pics/surfaceRefFrame.png}
    }
    \hfill
    \subfigure{
        \centering
        \includegraphics[width=\textwidth]{./pics/molaObservation.png}
	}
    \caption{After disabling the proxy geometry, the loaded OPC is visible.}
    \label{fig:disableProxy}
\end{figure}
...
It is possible to add new celestial bodies and new reference frames.

\hypertarget{extended-concept}{%
	\subsubsection{Extended concept}\label{extended-concept}}

For story telling, PRo3D also supports the creation GIS bookmarks.
Similarly to storytelling for surfaces, this can be used to create movies and
interactive presentations.

\hypertarget{caveats}{%
	\subsubsection{Caveats}\label{caveats}}

Currently, the GIS settings are not stored in the scene files.


\subsubsection{Mission Phases}
\label{sec:missionTimes}

The mission time settings, shown in Figure \ref{label:mission_phases} allow the user to choose a mission time frame and navigate through the simulation using the slider. The mission phases include the Deimos Flyby, the Mars Flyby and the Didymos Orbital Insertion.

Everything that depends on the observation time follows this slider, which includes the
illumination of the scene: with the \textit{Sun / Lighting Mode} set to \textit{SunDirect}
or \textit{SunShadow}, surfaces are shaded with the real sun position for the current
observation time, so scrubbing through time moves the terminator and the cast shadows
across the terrain. This requires a loaded SPICE kernel that covers the selected point in
time and a surface registered with a reference frame and a body in the GIS Surfaces tab.

\furtherreading{\onlinedoc{TimeAndSun.md}{TimeAndSun.md} --- a walkthrough of moving the
scene through time with the sun lighting turned on: the prerequisites, the three lighting
modes and what to check when the sun does not seem to move;
\onlinedoc{Pro3DTool-SunAngles.md}{Pro3DTool-SunAngles.md} --- computing incidence, emission
and phase angles for images from the command line.}

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{./pics/mission_interface.png}
	\caption{Mission Phases}
    \label{label:mission_phases}
\end{figure}

\subsubsection{Markers}
\label{sec:markers}

Celestial bodies can be annotated with markers as shown in Figure \ref{fig:markers_on_mars}. 

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{./pics/markers_on_mars.png}
	\caption{Markers on Mars}
    \label{fig:markers_on_mars}
\end{figure}

\subsubsection{Instrument projections}
\label{sec:instrumentProjections}

It is possible to project images that were captured with Spacecraft instruments onto celestial bodies. 
Metadata is needed for the projection, 
which is stored in a json file with the image name and the suffix .mbi, i.e. <image\_name>.mbi.json. 
It stores information about the instrument, the time and the target body. 
As this is part of an ongoing project, for a schema definition, please contact pro3d@vrvis.at.
...
A second json file with the format <image\_name>.json contains 
information about the image. 
We require this file to store information about the number of channels 
and the minimum and maximum pixel values that were used to capture the image.   

The images captured with the Hera instruments can be loaded 
using the main menu or the \textit{Projected Images} tab in the Gis panel, as shown in Figure \ref{fig:load_instrument_images}.


\begin{figure}[h!]
    \subfigure{
        \includegraphics[width=0.499\textwidth]{./pics/load_instrument_images_main_menu.PNG}
	}
    \subfigure{
        \includegraphics[width=0.499\textwidth]{./pics/load_instrument_images_gis_view.PNG}
	}
    \caption{Load Instrument Images}
    \label{fig:load_instrument_images}
\end{figure}

Figure \ref{fig:image_settings} shows the parameters the user can adjust after loading an image. The user can choose a channel using the \textit{Channel} parameter in case that a layered \textit{tiff} file is loaded. The \textit{false color visualization} can be activated to enhance properties and the corresponding dropdown offers different color maps to choose from. \textit{Minimum} and \textit{maximum} are set to the minimum and maximum values that were used by the instrument to capture the image. Those values are extracted from the json file that is delivered with the instrument image. The user is not limited to a range if they want use the text field to set a minimum and maximum value to map the color values to a new range. The sliders are set to the captured color range +/- 10\%.        

\begin{figure}[h!]
    \subfigure{
        \includegraphics[width=0.499\textwidth]{./pics/projected_images_settings_1.PNG}
    }
    \subfigure{
        \includegraphics[width=0.499\textwidth]{./pics/projected_images_settings_2.PNG}
	}
    \caption{Settings for Images}
    \label{fig:image_settings}
\end{figure}

Figure \ref{fig:image_projection} shows the settings the user can adjust to control the image projection onto a body. The \textit{visualization} slider controls the opacity of the image projection and the registration properties \textit{roll}, \textit{pitch} and \textit{yaw} register the orientation of the projected image. The visibility can be set to \textit{RelativeCount}. The sun / lighting mode provides the options \textit{Off, SunDirect} and \textit{SunShadow}, which shade the surfaces with the sun position of the current observation time (Section~\ref{sec:missionTimes}).

\furtherreading{\onlinedoc{MultiImageProjection.md}{MultiImageProjection.md} --- projecting
several instrument images at once and blending between them;
\onlinedoc{ProjectionValidation.md}{ProjectionValidation.md} --- checking whether a
projected image lands exactly on the terrain;
\onlinedoc{Pro3DTool-SimulateImage.md}{Pro3DTool-SimulateImage.md} and
\onlinedoc{Pro3DTool-Unproject.md}{Pro3DTool-Unproject.md} --- rendering and unprojecting
instrument images from the command line.}

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{./pics/projected_images_settings.png}
	\caption{Settings for Image Projection}
    \label{fig:image_projection}
\end{figure}


Figure \ref{fig:hera-projectedImages} and Figure \ref{fig:hera-projectedImages2} display the image projection onto Mars.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{./pics/loaded_images.png}
	\caption{Image Projection on Mars}
    \label{fig:hera-projectedImages2}
\end{figure}

\subsubsection{Ellipse annotations}
\label{sec:ellipseAnnotations}

Ellipse annotation (using the AxisEllipse) geometry type is inspired by SBMT elliptic annotations.

It is based on three points that the user sets:

\begin{itemize}

\item First point: End point of major axis
\item Second point: Second end point of major axis
\item Third point: Specifies length of the minor axis (normal distance to major axis)

\end{itemize}

The outline is fitted in the plane through these three points and then draped onto the
terrain along the scene's up vector. This is why ellipses always use the \textit{Sky}
projection and why they need a reference body: with no body set for the scene there is no
up vector to drape along, and the entry in the annotation dropdown stays greyed out with
the note ``needs a reference body''.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.6\textwidth]{./pics/AnnotationEllipse.PNG}
    \caption[AxisEllipse Annotation]{An AxisEllipse drawn on Dimorphos: the outline is fitted in the plane through the three picked points and then draped onto the terrain.}
    \label{fig:annotation_ellipse}
\end{figure}

\furtherreading{\onlinedoc{EllipseAnnotations.md}{EllipseAnnotations.md} --- both ellipse
tools and their parameters; \onlinedoc{SbmtImport.md}{SbmtImport.md} --- importing ellipses
and circles from SBMT annotation catalogs.}
...
\furtherreading{\onlinedoc{Contour-Lines.md}{Contour-Lines.md} --- the contour line feature
and its parameters.}
...
\furtherreading{\onlinedoc{Feature-Multitexture.md}{Feature-Multitexture.md} --- the
multitexturing feature in detail; \onlinedoc{Feature-Queries.md}{Feature-Queries.md} ---
the query parameters and transfer functions used to filter attributes;
\onlinedoc{VertexAttributes.md}{VertexAttributes.md} --- per-vertex attribute layers.}
