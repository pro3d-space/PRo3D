...
\begin{itemize}
	...
	\item \textit{AxisEllipse}: A three-point ellipse for marking roughly elliptical features such as craters, boulders, or instrument footprints.
	Pick two points \key{Ctrl}+\key{LMB} to define the \textbf{major axis} endpoints, then pick a third point to set the \textbf{semi-minor radius} as a perpendicular distance from the major axis.
	The resulting ellipse is always symmetric about the major axis.
	\item \textit{Axis4PEllipse}: A four-point ellipse that allows asymmetric and half-moon shapes.
	As with AxisEllipse, the first two points define the \textbf{major axis} endpoints.
	The third and fourth points each define a semi-minor extent on either side of the major axis:
	if both points are picked on the \textbf{same side}, the result is a half-moon shape;
	if picked on \textbf{opposite sides}, the ellipse is symmetric when the two distances are equal, or asymmetric when they differ.
\end{itemize}
